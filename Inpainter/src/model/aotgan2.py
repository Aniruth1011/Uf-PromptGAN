...
class AOTBlock(nn.Module):
        ...
        for i, rate in enumerate(rates):
            self.__setattr__(
                ...
                nn.Sequential(
                    ...
                    nn.ReLU(True)))
        self.prompt_local = PromptGenBlock()
        self.prompt_global = PromptGenBlock()
    ...
    def forward(self, x):
        prompt = self.prompt_global(x)
        out = [self.__getattr__(f'block{str(i).zfill(2)}')(torch.cat(x) , 1)) for i in range(len(self.rates))]
        out = torch.cat((out, prompt) 1))
        out = self.fuse(out)
